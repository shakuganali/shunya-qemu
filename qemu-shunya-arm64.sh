...
grid=$( id -g $user ) 
urid=$( id -u $user )
FILE=$DIR/rfs/shunya-console-image-qemuarm64.rootfs.ext4 
cd $DIR/rfs/  
if [ ! -f $FILE ]; then
	echo "Extracting the root filesystem"
	tar -xzf $DIR/rfs/shunya-console-image-qemuarm64.rootfs.ext4.tar.gz
fi
cd $DIR
	echo ""
	echo ""

./runqemu-ifup ${urid} ${grid} ./binaries/
